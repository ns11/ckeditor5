...
//set the build path
const build_path = path.join( ROOT_DIRECTORY, '..', '..', 'djangocms_text_ckeditor', 'static', 'djangocms_text_ckeditor', 'build' );
...
const webpackConfig = {
	...
	plugins: [
		...
		new webpack.DllPlugin( {
			name: 'CKEditor5.dll',
			context: 'src',
			path: path.join( build_path, 'ckeditor5-dll.manifest.json' ),
			format: true,
			entryOnly: true
		} )
	],
